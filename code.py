...
def splash_scene():
    ...
    # an image banks for CircuitPython
...
def game_scene():
    ...
    score_text.move(1,1)
    score_text.text(f"Score: {score} Lives {lives}")
    ...
    image_bank_background = stage.Bank.from_bmp16("game_scene_studio.bmp")
...
def game_over_scene(final_score):
    ...
    # create a stage from the background to show up on 
    #   and set the frame rate to 60fps
    game= stage.Stage(ugame.display, constants.FPS)
    # set the layers, items show up in order
    game.layers = text + [background]
    # render the backfround and intitial location of sprite list
